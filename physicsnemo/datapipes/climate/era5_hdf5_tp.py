...
class ERA5HDF5Datapipe(Datapipe):
    ...
    def parse_dataset_files(self) -> None:
        ...
        with h5py.File(self.data_paths[0], "r") as f:
            ...
            data_samples_per_year = (
                (f["tp"].shape[0] - self.num_steps * self.stride) // self.world_size
            ) * self.world_size
            self.img_shape = f["tp"].shape[1:]

            # If channels not provided, use all of them
            if self.channels is None:
                self.channels = [i for i in range(f["tp"].shape[1])]
            ...
            # Sanity checks
            if max(self.channels) >= f["tp"].shape[1]:
                raise ValueError(
                    f"Provided channel has indexes greater than the number \
                of fields {f['tp'].shape[1]}"
                )
            ...
            self.logger.info(f"Number of channels available: {f['tp'].shape[1]}")
    ...
    def _create_pipeline(self) -> dali.Pipeline:
        ...
        with pipe:
            ...
            # 调整形状，添加一个维度。
            invar = dali.fn.reshape(invar, shape=[-1, 1, self.img_shape[0], self.img_shape[1]])
            outvar = dali.fn.reshape(outvar, shape=[-1, 1, self.img_shape[0], self.img_shape[1]])
...
class ERA5DaliExternalSource:
    ...
    def __call__(self, sample_info: dali.types.SampleInfo) -> Tuple[Tensor, Tensor]:
        ...
        data = self.data_files[year_idx]["tp"]
        # Has [C,H,W] shape.
